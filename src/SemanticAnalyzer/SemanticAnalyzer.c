  ...
  if (astnode->parent->type != dot ||
    ...
    if (entry->tableType == VARIABLE_ENTRY ||
      ...
      if (entry->tableType == FPARAM_ENTRY)
        variableTypeInfo = entry->data.fparamEntry.type;
      else
      ...
      if (variableTypeInfo.numberofdims == varDimsCount) {
        ...
        curVarTypeInfo.typeString =
            strdup(variableTypeInfo
                       .typeString); // The type string, this is what is causing
                                     // the seg fault, how can we fix this?
        ...
        if (!variableTypeInfo.typeString)
          fprintf(stderr, "ERROR LINE 445 !!! STRDUP\n");
